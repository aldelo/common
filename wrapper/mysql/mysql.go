...
func (t *MySqlTransaction) Commit() (err error) {
	...
	if t._xrayTxSeg != nil {
		defer func() {
			...
			if err != nil {
				xray.LogXrayAddFailure("MySQL", t._xrayTxSeg.SafeAddError(err))
			}
			if t.closed {
				t._xrayTxSeg.Close()
			}
...
func (svr *MySql) openWithXRay() (err error) {
	...
	defer trace.Close()
	defer func() {
		xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("DB-Host", svr.Host))
		xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("DB-Database", svr.Database))
		xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("DB-UserName", svr.UserName))
		xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("DB-Charset", svr.Charset))
		xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("DB-Collation", svr.Collation))
		xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("DB-ConnectTimeout", svr.ConnectTimeout))
		xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("DB-ReadTimeout", svr.ReadTimeout))
		xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("DB-WriteTimeout", svr.WriteTimeout))

		if err != nil {
			xray.LogXrayAddFailure("MySQL", trace.SafeAddError(err))
		}
	}()
	...
	defer func() {
		if err != nil {
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddError(err))
		}
	}()
...
func (svr *MySql) Ping() (err error) {
	...
	if !xray.XRayServiceOn() {
		...
		defer trace.Close()
		defer func() {
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("Ping-Timestamp-UTC", time.Now().UTC()))
...
func (svr *MySql) Begin() (*MySqlTransaction, error) {
	...
	if !xray.XRayServiceOn() {
		...
		if tx, err := db.BeginTxx(subXSeg.Ctx, &sql.TxOptions{Isolation: 0, ReadOnly: false}); err != nil {
			// begin failed
			xray.LogXrayAddFailure("MySQL", subXSeg.SafeAddError(err))
			xray.LogXrayAddFailure("MySQL", xseg.SafeAddError(err))
			subXSeg.Close()
			xseg.Close()
			return nil, err
		} else {
...
func (svr *MySql) GetStructSlice(dest interface{}, query string, args ...interface{}) (notFound bool, retErr error) {
	...
	if !xray.XRayServiceOn() {
		...
		defer trace.Close()
		defer func() {
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("SQL-Query", query))
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("SQL-Param-Values", args))
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("Struct-Slice-Result", dest))
			if err != nil {
				xray.LogXrayAddFailure("MySQL", trace.SafeAddError(err))
			}
		}()
...
func (t *MySqlTransaction) GetStructSlice(dest interface{}, query string, args ...interface{}) (notFound bool, retErr error) {
	...
	if t._xrayTxSeg == nil {
		...
		defer subTrace.Close()
		defer func() {
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("SQL-Query", query))
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("SQL-Param-Values", args))
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("Struct-Slice-Result", dest))
			if err != nil {
				xray.LogXrayAddFailure("MySQL", subTrace.SafeAddError(err))
			}
		}()
...
func (svr *MySql) GetStruct(dest interface{}, query string, args ...interface{}) (notFound bool, retErr error) {
	...
	if !xray.XRayServiceOn() {
		...
		defer func() {
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("SQL-Query", query))
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("SQL-Param-Values", args))
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("Struct-Result", dest))
			if err != nil {
				xray.LogXrayAddFailure("MySQL", trace.SafeAddError(err))
			}
		}()
...
func (t *MySqlTransaction) GetStruct(dest interface{}, query string, args ...interface{}) (notFound bool, retErr error) {
	...
	if t._xrayTxSeg == nil {
		...
		defer func() {
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("SQL-Query", query))
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("SQL-Param-Values", args))
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("Struct-Result", dest))
			if err != nil {
				xray.LogXrayAddFailure("MySQL", subTrace.SafeAddError(err))
			}
		}()
...
func (svr *MySql) GetRowsByOrdinalParams(query string, args ...interface{}) (*sqlx.Rows, error) {
	...
	if !xray.XRayServiceOn() {
		...
		defer func() {
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("SQL-Query", query))
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("SQL-Param-Values", args))
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("Rows-Result", rows))
			if err != nil {
				xray.LogXrayAddFailure("MySQL", trace.SafeAddError(err))
			}
		}()
...
func (t *MySqlTransaction) GetRowsByOrdinalParams(query string, args ...interface{}) (*sqlx.Rows, error) {
	...
	if t._xrayTxSeg == nil {
		...
		defer func() {
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("SQL-Query", query))
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("SQL-Param-Values", args))
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("Rows-Result", rows))
			if err != nil {
				xray.LogXrayAddFailure("MySQL", subTrace.SafeAddError(err))
			}
		}()
...
func (svr *MySql) GetSingleRow(query string, args ...interface{}) (*sqlx.Row, error) {
	...
	if !xray.XRayServiceOn() {
		...
		defer func() {
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("SQL-Query", query))
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("SQL-Param-Values", args))
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("Row-Result", row))
			if err != nil {
				xray.LogXrayAddFailure("MySQL", trace.SafeAddError(err))
			}
		}()
...
func (t *MySqlTransaction) GetSingleRow(query string, args ...interface{}) (*sqlx.Row, error) {
	...
	if t._xrayTxSeg == nil {
		...
		defer func() {
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("SQL-Query", query))
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("SQL-Param-Values", args))
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("Row-Result", row))
			if err != nil {
				xray.LogXrayAddFailure("MySQL", subTrace.SafeAddError(err))
			}
		}()
...
func (svr *MySql) GetScalarString(query string, args ...interface{}) (retVal string, retNotFound bool, retErr error) {
	...
	if !xray.XRayServiceOn() {
		...
		defer func() {
			...
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("Row-Result", row))
			if retErr != nil {
				xray.LogXrayAddFailure("MySQL", trace.SafeAddError(retErr))
			}
		}()
...
func (t *MySqlTransaction) GetScalarString(query string, args ...interface{}) (retVal string, retNotFound bool, retErr error) {
	...
	if t._xrayTxSeg == nil {
		...
		defer func() {
			...
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("Row-Result", row))
			if retErr != nil {
				xray.LogXrayAddFailure("MySQL", subTrace.SafeAddError(retErr))
			}
		}()
...
func (svr *MySql) ExecByOrdinalParams(actionQuery string, args ...interface{}) MySqlResult {
	...
	if !xray.XRayServiceOn() {
		...
		defer trace.Close()
		defer func() {
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("SQL-Query", actionQuery))
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("SQL-Param-Values", args))
			xray.LogXrayAddFailure("MySQL", trace.SafeAddMetadata("Exec-Result", result))
			if err != nil {
				xray.LogXrayAddFailure("MySQL", trace.SafeAddError(err))
			}
		}()
...
func (t *MySqlTransaction) ExecByOrdinalParams(actionQuery string, args ...interface{}) MySqlResult {
	...
	if t._xrayTxSeg == nil {
		...
		defer subTrace.Close()
		defer func() {
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("SQL-Query", actionQuery))
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("SQL-Param-Values", args))
			xray.LogXrayAddFailure("MySQL", subTrace.SafeAddMetadata("Exec-Result", result))
			if err != nil {
				xray.LogXrayAddFailure("MySQL", subTrace.SafeAddError(err))
			}
		}()
